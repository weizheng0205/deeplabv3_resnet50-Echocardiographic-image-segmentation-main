...
class Echo(torchvision.datasets.VisionDataset):
    def __init__(self, root=None, split="train", target_type="EF", mean=0., std=1., length=16, period=2,
        ...
        #root 数据集根目录
        self.split = split.upper()      #数据划分
        if not isinstance(target_type, list):
            target_type = [target_type]
        self.target_type = target_type  #target_type 指定输出目标类型
        self.mean = mean    #视频标准化参数
        self.std = std  #视频标准化参数
        self.length = length    #输出剪辑的帧数
    ...
    def __getitem__(self, index):
        ...
        if f < length * self.period:
            # 帧数不足时补充，如果视频太短，就用0填充帧 0代表平均颜色（深灰色），因为这是在标准化之后
